...
function HomePage() {
  ...
  return (
    <Container fluid className="p-10"> {/* Use fluid to ensure full width and set padding to 0 */}
      <Row className="mb-0"> {/* Remove bottom margin from the row */}
        <Col md={4} className="p-0"> {/* Remove padding from the column */}
          <h2>Categories</h2>
          <ListGroup>
        ...
        <Col md={8} className="mb-4 p-0"> {/* Remove padding from the column */}
          <Carousel>
            {products.slice(0, 5).map((product) => (
              <Carousel.Item key={product.id}>
                <Link to={`/product/${product.id}`} style={{ textDecoration: 'none', color: 'inherit' }}> {/* Link wrapping Carousel.Item */}
        ...
        {products.map((product) => (
          <Col md={3} sm={6} xs={12} key={product.id} className="mb-3">
            <Card className="product-card">
              {/* Wrap the entire card with a Link to navigate to the product details */}
